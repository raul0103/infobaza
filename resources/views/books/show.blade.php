...
<x-page-header :title="$book->title" :subtitle="trim(($book->author ?? '').($book->year ? ', '.$book->year : ''))">
    <x-slot:actions>
        ...
        <a href="{{ route('books.edit', $book) }}" class="btn btn-secondary">Изменить</a>
        @include('partials.delete-form', ['action' => route('books.destroy', $book)])
    </x-slot:actions>
</x-page-header>
@if($book->status === 'reading' && $book->total_pages)
<div class="card mb-6">
    <div class="flex justify-between text-sm mb-2"><span class="text-gray-600">Прогресс чтения</span><span class="font-medium">{{ $book->current_page ?? 0 }} / {{ $book->total_pages }} ({{ $book->readingPercent() }}%)</span></div>
    <div class="h-3 bg-gray-100 rounded-full overflow-hidden"><div class="h-full bg-emerald-500 rounded-full" style="width:{{ $book->readingPercent() }}%"></div></div>
</div>
@endif
...
<div class="space-y-4">
    @forelse($book->quotes as $quote)
        <div class="card border-l-4 border-l-blue-500">
            <blockquote class="text-lg text-gray-800 italic leading-relaxed">«{{ $quote->text }}»</blockquote>
            ...
            <div class="mt-4 flex gap-3 pt-3 border-t border-gray-100">
                <a href="{{ route('quotes.edit', $quote) }}" class="link">Изменить</a>
                @include('partials.delete-form', ['action' => route('quotes.destroy', $quote)])
            </div>
        </div>
    @empty
